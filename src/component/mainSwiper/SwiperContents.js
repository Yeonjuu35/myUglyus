        ...
        <div style={{ backgroundColor: info["bgcolor"] }} className='d-flex align-items-center swiperImg'>
            <div className='container customWidthS d-flex align-items-center flex-column flex-lg-row'>
                <div className='col-lg-6 d-flex flex-column ps-lg-5 pb-5 pb-lg-0 scaleCustom'>
                    <h2 className='fontMainSwiper pb-lg-3 pb-2' style={{ color: fontColor }}>
                        {titleArray.map((v, i) => (
                            i === 0 
                            ? <span key={i}>{v}</span>
                            : <span key={i} className='fontBold d-block'>{v}</span>
                        ))}
                    </h2>
                    <Leadcopy className='pb-lg-5 pb-3' style={{ color: fontColor }}>
                ...
                <div className='col-lg-6 position-relative'>
                    {info["swipertag"] && (
                        <TagType2 style={{ background: info["swipertagcolor"] }} className='swipertagStyle'>
                            {info["swipertag"]}
                        </TagType2>
                    )}
